...
urlpatterns = [
    ...
    path('',views.inicio),            # Envía al usuario a la función homepage, que se encuentra en views.py
]
...
Ahora debería correr correctamente tanto la página inicio.html, como la de about.html.
...
CRUD: alta, baja, modificación y eliminación de ejercicios


Para organizar un poco nuestro proyecto, crearemos en principio la aplicación de muestra de ejercicios:

Iremos creando una a una todas las aplicaciones. La aplicación base physicsEval fue creada al principio del proyecto. Ahora crearemos showEjer. En el directorio raíz, el mismo que contiene a manage.py, correremos:
$ python manage.py startapp showEjer

Este comando creará una carpeta ejercicios, que sí traerá incorporado un archivo views.py; es decir que no lo tendremos que crear nosotros como lo hicimos anteriormente.
